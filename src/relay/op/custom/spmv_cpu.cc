...
namespace tvm {
namespace relay {
TVM_REGISTER_NODE_TYPE(CustomSpmv_CpuAttrs);
bool Spmv_Cpu_equalRel(const Array<Type>& types, int num_inputs, const Attrs& attrs, const TypeReporter& reporter) {
  // types: [data, output]
...
Expr MakeSpmv_Cpu(Expr S_M,Expr V) {
  auto attrs = make_object<CustomSpmv_CpuAttrs>();
  static const Op& op = Op::Get("custom.spmv_cpu");
  return Call(op, {S_M,V}, Attrs(attrs), {});
}

TVM_REGISTER_GLOBAL("relay.op.custom._make.spmv_cpu").set_body_typed(MakeSpmv_Cpu);


RELAY_REGISTER_OP("custom.spmv_cpu")
    .describe(R"code(spmv cpu)code" TVM_ADD_FILELINE)
    .set_attrs_type<CustomSpmv_CpuAttrs>()
    .set_num_inputs(2)
    .add_argument("S_M", "2D Tensor", "Input data1.")
    .add_argument("V", "1D Tensor", "Input data2.")
    .set_support_level(1)
    .add_type_rel("spmv_cpu", Spmv_Cpu_equalRel)
