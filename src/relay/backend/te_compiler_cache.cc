...
class LowerToTECompute : public backend::MemoizedExprTranslator<Array<te::Tensor>> {
 public:
  explicit LowerToTECompute(Target target, NameSupply constants_name_supply, bool use_tvm_script_kernel=false)
      : target_(target), device_copy_op_(Op::Get("device_copy")),bypass_te(use_tvm_script_kernel),constants_name_supply_(constants_name_supply)  {}
...
class ScheduleBuilder : public ExprVisitor {
 public:
  explicit ScheduleBuilder(Target target)
    ...
    if (backend::IsMetaScheduleEnabled()) {
      ...
      CHECK(database_.defined()) << "ValueError: `use_meta_schedule` is enabled in Relay "
                                             "build, but no Database context is provided. ";
  ...
  CachedFunc Create(const Function& relay_func, GlobalVarSupply global_var_supply,
    ...
    LowerToTECompute lower_te_compute(target_, constant_name_supply, use_tvm_script_kernel);
