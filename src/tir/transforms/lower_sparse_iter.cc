...
class IterTransformer : public StmtExprMutator {
 ...
 private:
  ...
  Stmt GenerateLoops(Stmt body, const Array<IterVar>& block_iters,
    ...
    // std::cout << "block_iters:" << PrettyPrint(block_iters) << std::endl;
    // std::cout << "iter_bindings:" << PrettyPrint(iter_bindings) << std::endl;
  ...
  Stmt VisitStmt_(const SparseIterationNode* sp_iteration) final {
    ...
    // std::cout << "before substitute:" << PrettyPrint(sp_iteration->body) << std::endl;
    // std::cout << "var_map:" << PrettyPrint(var_map) << std::endl;
    Stmt body = VisitStmt(Substitute(sp_iteration->body, var_map));
    // std::cout << "after substitute:" << PrettyPrint(body) << std::endl;
...
PrimFunc LowerSparseIter(PrimFunc f, bool check_invalid_binary_search) {
  ...
  if (!IsFromLegacyTESchedule(f) && SparseTIRLevel(f) == 2) {
    ...
    // std::cout << "before lower iter:" << PrettyPrint(fptr->body) << "\n";
    ...
    Block root_block({}, {}, {}, "root", body, NullOpt, lower_sparse.root_alloc_buffers, {},
                     Map<String, ObjectRef>(), Span(), buf_doms);
