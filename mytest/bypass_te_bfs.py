...
if(compile_needed):
  ...
  def body(Pi, W,i_tensor, i):
      ...
      G_C= relay.annotation.on_device(G_after_split[0],tvm.cpu(),constrain_body=False,constrain_result=True)
      ...
      W_c = relay.annotation.on_device(relay.custom.spmv_cpu(G_C,W),tvm.cpu())
      W_n = relay.annotation.on_device(relay.custom.concat_vector(S_info,W_c,W_g),tvm.cpu())
      Pi_n = relay.annotation.on_device(relay.custom.bfs_pprocess(W_n,Pi,i_tensor),tvm.cpu())
      j = relay.add(i, relay.const(1, "int32"))
      i_tensor_n = relay.add(i_tensor, relay.const(1, "int32"))
      return Pi_n,W_n,i_tensor_n, j
