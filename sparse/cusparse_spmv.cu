...
//./spmv_cusparse -f csr -m /home/v-yinuoliu/yinuoliu/code/SparseCodegen/matrix2388

// /usr/local/cuda-12.1/bin/nvcc -O3 -arch=sm_80 -lineinfo -o ${1} ${1}.cu -l cusparse
// nvcc -O3 -arch=sm_80 -lineinfo cusparse_spmv.cu -o cusparse_spmv -l cusparse
using namespace std;
...
void readVerEdges(int &is_weighted, int &n, int &t, int &m, std::string &file)
{
    ...
    if(cnt == 1){
        ...
        std::cout<<"error! you need to get right mtx input2\n";
        exit(0);
    }
    input.close();
}
...
void readMtxFile(int is_weighted, int n, int m,
            ...
            std::string &file,const std::string &format)
{
    ifstream input;
    input.open( file + ".mtx");
    ...
    int idx = 0;
    if (format == "coo") {
        std::vector<std::pair<std::pair<int,int>,float>> tmp_vector;
        if(is_weighted == 0){
            while (input >> a >> b)
            {
                a--;
        ...
        for (auto i : tmp_vector) {
            row_offset[idx] = i.first.first;
            col_index[idx] = i.first.second;
            val[idx] = i.second;
            idx++;
        }
        return;
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        std::sort(col_index+start_index,col_index+nnz_num);
...
template<typename T>
void vec_print(vector<T> array,int size){
    int counter = 0;
    for(auto x: array){
...
int main(int argc, char **argv)
{
    ...
    //parse arg
    if (argc == 1) {
        // file = "/home/v-yinuoliu/yinuoliu/code/TVMSparseCodegen/matrix933";
        file = "/home/v-yinuoliu/yinuoliu/code/SparseCodegen/matrix2388";
        // file = "/home/v-yinuoliu/yinuoliu/code/TVMSparseCodegen/matrix35";
        using_format = "csr";
        printf("%s.mtx\n",file.c_str());
        ...
        if (strcmp(argv[1], "-m") == 0) {
            ...
            printf("get argv[1]:%s\n",argv[1]);
            printf("usage: ./spmv -m [matrix_path] -f [format]\n");
            exit(0);
        }
    }
    else if (argc == 5) {
    ...
    printf("using sparse format:%s \n",using_format.c_str());
    vector<int> row_offset;
    if( using_format == "coo") {
        row_offset.resize(nnz_num);
    } else {
        row_offset.resize(row_num + 1);
    }
    ...
    int iter = 1000;
    int warmup = 200;
    readMtxFile(is_weighted, row_num, nnz_num, row_offset.data(), col_index.data(), value.data(), file, using_format);

    // check input
    // std::cout<<" The row_offset is: "<<std::endl;
    // vec_print<int>(row_offset, 100);
    // std::cout<<" The col_index is: "<<std::endl;
    // vec_print<int>(col_index, 100);
    // std::cout<<" The value is: "<<std::endl;
    // vec_print<float>(value, 100);
    ...
    float* d_y_cusparse;
    if( using_format == "coo") {
        checkCudaErrors(cudaMalloc(&d_A_row_offset, (nnz_num)*sizeof(int)));
        checkCudaErrors(cudaMemcpy( d_A_row_offset, row_offset.data(), (nnz_num)*sizeof(int), cudaMemcpyHostToDevice));
    } else {
    ...
    if(using_format == "sell") {
        // Create sparse matrix A in SlicedEll format
        CHECK_CUSPARSE(cusparseCreateSlicedEll(&matA, row_num, col_num, nnz_num,
                                            1,1,
                                            d_A_row_offset, d_A_col_index, d_A_value,
                                            CUSPARSE_INDEX_32I, CUSPARSE_INDEX_32I,
                                            CUSPARSE_INDEX_BASE_ZERO, CUDA_R_32F));
    }
    else if(using_format == "csr") {
    ...
    else if(using_format == "coo") {
        CHECK_CUSPARSE( cusparseCreateCoo(&matA, row_num, col_num, nnz_num,
                                        d_A_row_offset, d_A_col_index, d_A_value,
                                        CUSPARSE_INDEX_32I,
                                        CUSPARSE_INDEX_BASE_ZERO, CUDA_R_32F) )
    }
    ...
    // vec_print<float>(y_cusparse_res,100);
